...
export class OrthographicCameraControls {
  ...
  constructor(object: OrthographicCamera, domElement, private storageService: StorageService) {
    ...
    console.log(this.target.angleTo(this.object.position));
    console.log(this.object.position.z / this.radius + " " + this.object.position.x / (this.radius * Math.sin(this.theta)))
    this.theta = Math.acos(this.object.position.z / this.radius);
    console.log(this.theta + " theta");
    this.phi = Math.atan2(this.object.position.y, this.object.position.x);
