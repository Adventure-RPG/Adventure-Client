import { Geometry, Line, LineBasicMaterial, Object3D, Points, PointsMaterial, Vector3 } from "three";
...
*/
  //TODO: change any to Material[]
  static drawThreeGeo(json, radius: number, shape: 'sphere' | 'plane', materalOptions: {color: any}, container: Object3D) {
    ...
    function drawParticle(x, y, z, options) {
      let particle_geom = new Geometry();
      particle_geom.vertices.push(new Vector3(x, y, z));

      let particle_material = new PointsMaterial(options);

      let particle = new Points(particle_geom, particle_material);
    ...
    function drawLine(x_values, y_values, z_values, options) {
      let line_geom = new Geometry();
      createVertexForEachPoint(line_geom, x_values, y_values, z_values);

      let line_material = new LineBasicMaterial(options);
      let line = new Line(line_geom, line_material);
      container.add(line);
